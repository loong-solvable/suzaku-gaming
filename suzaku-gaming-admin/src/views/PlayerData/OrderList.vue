...
// 表格列配置 - 按目标顺序
const tableColumns = [
  { prop: "project", label: "项目", width: 70 },
  { prop: "roleId", label: "角色ID", minWidth: 100 },
  { prop: "serverId", label: "区服", width: 60 },
  { prop: "deviceType", label: "系统", width: 70 },
  { prop: "roleName", label: "角色昵称", minWidth: 90 },
  { prop: "roleLevel", label: "角色等级", width: 80 },
  { prop: "payTime", label: "充值时间", minWidth: 140, sortable: true },
  { prop: "lastLoginTime", label: "最后登录时间", minWidth: 140, sortable: true },
  { prop: "payAmountUsd", label: "充值金额", width: 90, sortable: true },
  { prop: "currency", label: "币种", width: 60 },
...
const totalAmountDisplay = computed(() => {
  return summaryData.value.totalAmount.toFixed(2);
});
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await fetch("/api/player/orders?" + params.toString());
    ...
    if (json.code === 0) {
      ...
      // 更新统计数据
      summaryData.value = {
        totalAmount: json.data.summary?.totalAmount || 0,
        totalCount: json.data.summary?.totalCount || 0
      };
    }
  } catch (error) {
...
<template>
  <div class="order-list-page">
    ...
    <div class="summary-row">
      <span class="summary-text">累计充值金额: <strong>{{ totalAmountDisplay }}</strong></span>
    </div>
      ...
      >
        ...
        >
          <template #default="{ row }">
            <template v-if="col.prop === 'payAmountUsd'">
